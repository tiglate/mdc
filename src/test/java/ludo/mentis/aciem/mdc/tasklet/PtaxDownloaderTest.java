...
class PtaxDownloaderTest {
    ...
    private PtaxDownloader tasklet;
    ...
    void setUp() throws Exception {
        // Set up test data
        var serviceUrl = "https://example.com/ptax?startDate=%s&endDate=%s";
    ...
    void execute_shouldDownloadFileAndPopulateJobContext() throws Exception {
        ...
        verify(fileDownloadService).downloadFile(any(URL.class));
    ...
    void execute_shouldUseCorrectUrl() throws Exception {
        // Given
        var customUrl = "https://custom.example.com/api/ptax?startDate=%s&endDate=%s";
        var formattedUrl = PtaxDownloader.constructDownloadUrl(customUrl);
        tasklet = new PtaxDownloader(fileDownloadService, customUrl);

        // When
        tasklet.execute(stepContribution, chunkContext);

        // Then
        verify(fileDownloadService).downloadFile(new URL(formattedUrl));
    }
}
